...
activeItem = signal('Dashboard');
  ...
  mainMenu = [
    ...
    { name: 'Notes', icon: this.getIconSvg('calendar'), url:'teacher/notes' },
    { name: 'Send Notification', icon: this.getIconSvg('messages'),url:'teacher/notification' },
  ];

  academicMenu = [
    // { name: 'Registration', icon: this.getIconSvg('registration') },
    // { name: 'Students', icon: this.getIconSvg('students') },
    // { name: 'Library', icon: this.getIconSvg('library') },
    // { name: 'Courses', icon: this.getIconSvg('courses') },
    // { name: 'Professors', icon: this.getIconSvg('professors') },
    // { name: 'Department', icon: this.getIconSvg('department') },
    // { name: 'Financial Record', icon: this.getIconSvg('financial') },
  ];

  setActiveItem(itemName: string,itemUrl:string) {
    this.teacherroute(itemUrl);
    this.activeItem.set(itemName);
  }


  getMenuItemClasses(itemName: string): string {
  ...
  getIconSvg(name: string): string {
    const icons: { [key: string]: string } = {
      ...
      courses: `<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M3.75 9.776c.112-.017.227-.026.344-.026h15.812c.117 0 .232.009.344.026m-16.5 0a2.25 2.25 0 00-1.883 2.542l.857 6a2.25 2.25 0 002.227 1.932H19.05a2.25 2.25 0 002.227-1.932l.857-6a2.25 2.25 0 00-1.883-2.542m-16.5 0A2.25 2.25 0 015.625 7.5h12.75c1.135 0 2.165.755 2.542 1.883" /></svg>`,
      professors: `<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M15.75 6a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0zM4.501 20.118a7.5 7.5 0 0114.998 0A17.933 17.933 0 0112 21.75c-2.676 0-5.216-.584-7.499-1.632z" /></svg>`,
      department: `<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M2.25 21h19.5m-18-18v18m10.5-18v18m6-13.5V21M6.75 6.75h.75m-.75 3h.75m-.75 3h.75m3-6h.75m-.75 3h.75m-.75 3h.75M9 21v-3.375c0-.621.504-1.125 1.125-1.125h3.75c.621 0 1.125.504 1.125 1.125V21" /></svg>`,
      financial: `<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M2.25 18.75a60.07 60.07 0 0115.797 2.101c.727.198 1.453-.342 1.453-1.096V18.75M3.75 4.5v.75A.75.75 0 013 6h-.75m0 0v-.75A.75.75 0 013 4.5h.75m0 0a.75.75 0 00.75.75h7.5a.75.75 0 00.75-.75V4.5m0 0a.75.75 0 00-.75-.75h-7.5a.75.75 0 00-.75.75M15 4.5v.75A.75.75 0 0114.25 6h-.75m0 0v-.75a.75.75 0 01.75-.75h.75m0 0a.75.75 0 00.75.75h3a.75.75 0 00.75-.75V4.5m0 0a.75.75 0 00-.75-.75h-3a.75.75 0 00-.75.75M3 12h18M3 15h18" /></svg>`,
    };
    return icons[name] || ''; // Return empty string if icon not found
  }
}
